...
class Lang
{
    ...
    public static function indexDetect()
    {
        $langSet = '';
        if (isset($_COOKIE[self::$langCookieVar])) {
            // Cookie 中设置了语言变量
            $langSet = $_COOKIE[self::$langCookieVar];
        } elseif (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
            // 自动侦测浏览器语言
            preg_match('/^([a-z\d\-]+)/i', $_SERVER['HTTP_ACCEPT_LANGUAGE'], $matches);
            $langSet = strtolower($matches[1]) ?? 'en-US';
            $langArr = explode('-', $langSet);
            switch ($langArr[0]) {
                case 'zh':
                    if (in_array($langArr[1], ['tw', 'hk'])) {
                        $langSet = 'zh-CHT';
                    } else {
                        $langSet = 'zh-CHS';
                    }
